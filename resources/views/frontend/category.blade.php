...
@section('title')
	<title>{{$cat->title}}|NEWS SITE</title>
	@stop
@section('content')
	<div class="wrapper">
			...
			<div class="col-md-8">
				<div class="col-md-12" style="padding:15px 15px 30px 0px;">
					...
					<h3 style="border-bottom:3px solid #81d742; padding-bottom:5px;">
						<span style="padding:6px 12px; background:#81d742; text-transform:upercase;">{{$cat->title}}</span></h3>
					</div>
					@if(count($posts)>0)
					<div class="col-md-12">
						...
						<img src=" {{url('public/posts')}}/{{$post->image}}" width="100%" style="margin-bottom:15px;" /></a>
						{!! substr($post->description,0,300) !!}
						<p><a href="{{url('article')}}/{{$post->slug}}">
							Read more &raquo;</a></p>
					@endif
					@endforeach			   
					</div>
